...
> **Status: mid-v0 (v0.9.3).** The core engine and reconciler are real and shipping. CPU-only. SGD + Adam + AdamW + PyTorch-style SGD momentum (classical + Nesterov + dampening) supported. External framework traces are hand-authored sidecars today; v0.8 added multi-step observer-mode ingestion via JSONL streams; v0.9 added batched (multi-sample) observer-mode ingestion; v0.9.1 added Adam + AdamW with decoupled weight decay (Loshchilov & Hutter 2017 arXiv:1711.05101 Alg 2 line 12); v0.9.2 added classical PyTorch-style SGD momentum (Sutskever et al. 2013 ICML; `buffer_t = mu * buffer_{t-1} + gradient`; `lr` outside the buffer); **v0.9.3 widens to Nesterov accelerated gradient + dampening** (PyTorch `torch.optim.SGD` recurrence: `buffer_t = mu * buffer_{t-1} + (1 - tau) * gradient`; `effective = gradient + mu * buffer_t` when nesterov else `buffer_t`; `update = lr * effective`). v0.9.3 mirrors PyTorch's `nesterov=true && dampening>0` ValueError at both schema and engine boundaries. SGD coupled L2 weight decay deferred to v0.10. See [What's not in this version (yet)](#whats-not-in-this-version-yet) before you pick this up for production work.
...
- `schemas/` — receipt v0.1.0 / v0.2.0 / v0.3.0 / v0.4.0 / v0.5.0 / v0.6.0 / v0.7.0, topology-input v0.4.0, framework-trace v0.1.0 / v0.2.0 / v0.3.0 / v0.4.0 / v0.5.0 / v0.6.0 (all closed, `x-order`-annotated, additive)
