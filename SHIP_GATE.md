...
- [x] `[all]` README is current (last full rewrite 2026-05-17 for the v0.7.0 surface; re-anchored through v0.12.0). Surface accurate: **26 rules**, the full subcommand set, softmax+CE / external trace ingestion / observer-mode receipts / PyTorch+JAX+TensorFlow import / Adam+AdamW+SGD-momentum / batching / the live PyTorch helper all documented. Explicit Threat model section (now including v0.12.0 tolerance-gaming + provenance-laundering in-scope entries). Honest "What's not in this version (yet)" section names the remaining v1.0 product-completeness gaps (real-world fixture + adopter validation) in plain English.
...
- [ ] `[cli]` SKIP: Logging levels (silent / normal / verbose / debug) — `--verbose` ships; formal silent/debug deferred to v1.0.x — diagnostic need covered. `--verbose` / `-V` provides the diagnostic surface; the verifier never logs secrets (none exist in the system), so redaction is N/A and there is no operational driver for distinct silent/debug flags before v1.0.x. Not a hard-gate blocker.
- [ ] `[mcp]` SKIP: not an MCP server.
...
- Re-confirms shipcheck hard gates A–D within the explicit v0.x scope
- Closes 5 CRITICAL false-PASS holes from the dogfood-swarm audit: verifier-owned tolerance ceilings (a receipt can no longer widen its own pass band), marker-coupled Rule 14 (no relabel/strip bypass of the only math gate on observer imports), multi-step self-skip now NON-PASS, Rule 14 completeness assertion, and a Rule 0 reject for unrecognized optimizer names
- Adds proactive resource caps (`MAX_BATCH_SAMPLES`, `TOPOLOGY_SIZE_CEILING`) with actionable "limit exceeded" messages and graceful large-input handling (no raw stacks / OOM / hang)
- 12 trust invariants now hold with non-vacuous tests; suite grows 502 → 792 tests, deterministic

## What v0.12.0 does NOT do

- Does **not** promote to v1.0.0
- Does **not** add a real-world (CNN / transformer) hero fixture — the remaining v1.0 blocker on the product side
- Does **not** add adopter validation — the other remaining v1.0 blocker
- Does **not** add SGD coupled-L2 weight decay (the documented Rule 7 third branch — targeted v0.13)
- Does **not** ship the JAX live helper (targeted v1.0; `jax.make_jaxpr(grad)` trust boundary) or a pip distribution for the PyTorch helper
- Does **not** add GPU / fused-kernel bit-determinism (permanently out of scope — FP non-associativity, arXiv:2408.05148)

> **Historical note (v0.7.0 slice).** The v0.7.0 artifact-hygiene slice did NOT add multi-step observer-mode receipts, optimizers beyond SGD, the batch dimension, live framework helpers, a landing page, or a Starlight handbook — those all shipped in the subsequent v0.8–v0.11 slices and are present today. Preserved here for the audit trail; not a current-state statement.
