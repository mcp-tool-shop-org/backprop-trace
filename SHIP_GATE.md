...
- [ ] `[org]` Landing page (@mcptoolshop/site-theme) — DEFERRED. Phase 3 of full-treatment, after v1.0.0 product gaps close. Doesn't block v0.7.x.
...
| ~~Optimizers beyond vanilla SGD.~~ ~~No Adam, AdamW, momentum, or weight decay.~~ | ~~Real ML training in 2026 is overwhelmingly Adam by default. SGD-only verifiers won't get used for anything past coursework. Minimum bar: Adam or AdamW; momentum + weight decay an acceptable subset for "we don't have full Adam yet but the optimizer surface is real."~~ | **MOSTLY CLOSED in v0.9.2** — v0.9.1 shipped Adam + AdamW; v0.9.2 ships **classical PyTorch-style SGD momentum** via receipt.v0.6.0 + framework-trace.v0.5.0 forced bump + `OptimizerConfig.momentum` + `MomentumState = { buffer }` + Rule 21 (21a buffer recurrence + 21b parameter update) + generalized Rules 20, 25, 26. Sutskever et al. 2013 ICML / PyTorch `torch.optim.SGD` convention (`lr` outside the buffer; Rule 26 excludes `learning_rate` from constancy so LR schedules stay legitimate). v0.9.2 ships CLASSICAL ONLY — Nesterov accelerated gradient + dampening RESERVED for v0.9.3 (schema fields `nesterov: const false`, `dampening: const 0`); SGD coupled L2 weight decay deferred to v0.10 (Rules 6/7 third branch). |
| ~~Momentum SGD.~~ ~~Production ResNet-family training still uses SGD+momentum (Wilson et al. 2017 arXiv:1705.08292).~~ | ~~A v1.0 verifier covering "optimizers used in 2026 production ML" must cover momentum SGD alongside Adam.~~ | **CLOSED in v0.9.2** — classical PyTorch-style SGD momentum shipped. Nesterov is a v0.9.3 follow-on (schema slot reserved). |
| **Nesterov accelerated gradient + dampening (SGD momentum variants).** v0.9.2 ships classical PyTorch-style momentum; Nesterov (lookahead form `update = -lr * (mu * buf_after + gradient)`) reserved for v0.9.3. timm uses SGD+Nesterov as standard option for vision (Wilson et al. 2017 arXiv:1705.08292 §5 documents the generalization advantage). Schema reserves `nesterov: const false` + `dampening: const 0` so v0.9.3 widens to `boolean` / `number` without breaking v0.9.2 fixtures. | A v1.0 verifier covering production vision training (ResNet/EfficientNet via timm) needs Nesterov. Rule 21 widens to branch on `optimizer_config.nesterov`; one new fixture; +1 schema slot widen. | v0.9.3 |
...
| **Live framework helpers.** The framework-trace sidecar is hand-authored today. There is no `pip install @mcptoolshop/backprop-trace-pytorch` package, no `scripts/python-helpers/dump_pytorch_trace.py` ready-to-run extractor. | The path from "I have a PyTorch training step" to "I have a verified receipt" is: read schema → set up Python → write extractor by hand → match canonical-numeric format → emit JSONL. Almost nobody will do this. Without a live helper, external ingestion is a docs-grade workflow, not a real one. | v0.10 |
