...
| **Batch dimension.** Single-sample only. No batched forward / backward. | Real PyTorch / JAX / TF training is batched. A user with their actual production training step cannot import it without manually unrolling per-sample — which defeats the workflow. | v0.9 |
