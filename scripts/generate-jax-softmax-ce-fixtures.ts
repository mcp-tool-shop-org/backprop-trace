/**
 * v0.6.1 — Generate the canonical JAX softmax+CE fixture pair:
 *   - fixtures/external/jax.softmax-ce.sidecar.jsonl
 *       (framework-trace.v0.1.0 sidecar — what a JAX user would emit
 *        from their training loop via a thin Python helper that flattens
 *        jax.tree_util.tree_flatten(params) to {param_id: value})
 *   - fixtures/external/jax.softmax-ce.golden.jsonl
 *       (observer-mode v0.4.0 receipt — what `bp import jax` produces)
 *
 * Topology choice: same 2-2-3 softmax+CE as the PyTorch fixture but with
 * different weights + inputs + targets so the JAX golden is byte-distinct
 * from the PyTorch golden. This is the v0.6.1 pressure test — same shape,
 * same trust model, different framework + different topology values.
 *
 * Sidecar's claimed math = engine's recomputed math (v0.6.1 doesn't yet
 * have real JAX in CI; the canonical fixture demonstrates shape
 * correctness). A real-JAX-authored sidecar would carry minor FP drift
...
const PINNED_JAX_VERSION = "0.4.30"

// JAX 2-2-3 softmax+CE topology. Different weights + sample than the
// PyTorch fixture so the JAX golden is structurally distinct.
const JAX_SOFTMAX_CE_TOPOLOGY: Topology = {
...
const JAX_SOFTMAX_CE_INPUT: GeneralInput = {
  topology: JAX_SOFTMAX_CE_TOPOLOGY,
  learning_rate: 0.25,
  // Sample: (x1=0.5, x2=1.0), one-hot target class o2 (vs PyTorch fixture
  // which used class o1 — distinct labels).
  inputs: { x1: 0.5, x2: 1.0 },
  targets: { o1: 0, o2: 1, o3: 0 },
  parameters_before: {
    w_x1_h1: 0.15, w_x2_h1: 0.25, w_x1_h2: 0.35, w_x2_h2: 0.45,
    w_h1_o1: 0.20, w_h2_o1: 0.30,
    w_h1_o2: 0.40, w_h2_o2: 0.50,
    w_h1_o3: 0.10, w_h2_o3: 0.20,
  ...
  bias_policy: {
    mode: "constant",
    reason: "v0.6.1 JAX fixture pins biases as constant on step 1 — matches Mazur convention.",
    updated_in_step: false,
    reconciliation: "parameters_after[bias_id] === parameters_before[bias_id] for every bias parameter",
  },
  fixture: "jax-softmax-ce-engine-recompute",
  metadata: {
    source: "src/import-jax.ts (JAX softmax+CE observer-mode engine recompute reference)",
    gradient_convention: "descent_direction",
  },
}

// ---- Step 1: produce the engine receipt as the foreign-claim baseline.
const engineReceipt = runGeneralStep(JAX_SOFTMAX_CE_INPUT)

// ---- Step 2: build the sidecar. source_framework declares jax + a
// pinned version. The sidecar's claimed math is byte-identical to engine
// output for the v0.6.1 demo (real JAX would carry minor FP drift).
const sidecar = {
  format: "framework-trace.v0.1.0",
  source_framework: {
    name: "jax",
    version: PINNED_JAX_VERSION,
    information_uri: "https://docs.jax.dev/",
    extractor: {
      name: "bp-import-jax-helper",
      version: "0.6.1",
    },
  },
  topology: JAX_SOFTMAX_CE_INPUT.topology,
  learning_rate: JAX_SOFTMAX_CE_INPUT.learning_rate,
  numeric_policy: JAX_SOFTMAX_CE_INPUT.numeric_policy,
  bias_policy: JAX_SOFTMAX_CE_INPUT.bias_policy,
  inputs: JAX_SOFTMAX_CE_INPUT.inputs,
  targets: JAX_SOFTMAX_CE_INPUT.targets,
  parameters_before: JAX_SOFTMAX_CE_INPUT.parameters_before,
...
writeFileSync("fixtures/external/jax.softmax-ce.sidecar.jsonl", sidecarBytes)
console.log("wrote fixtures/external/jax.softmax-ce.sidecar.jsonl")

// ---- Step 3: run the importer on the sidecar. PINNED_TIMESTAMP makes
// the produced receipt deterministic across re-runs.
const result = importJaxSidecar(sidecarBytes, {
  importTimestamp: PINNED_TIMESTAMP,
  fixtureLabel: "jax-softmax-ce-imported",
})
...
console.log("wrote fixtures/external/jax.softmax-ce.golden.jsonl")
