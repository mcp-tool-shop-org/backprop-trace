...
const RULE_LABELS: Record<number, string> = {
  ...
  21: "PyTorch-style SGD momentum recurrence + effective direction + parameter update (Sutskever et al. 2013 ICML / PyTorch torch.optim.SGD; 21a: buffer_after == momentum * buffer_before + (1 - dampening) * gradient; 21b: effective == (gradient + momentum * buffer_after) if nesterov else buffer_after; 21c: update == learning_rate * effective; descent direction; v0.9.3 widens v0.9.2's classical-only to full PyTorch torch.optim.SGD surface — Nesterov + dampening accepted; nesterov=true + dampening>0 rejected mirroring PyTorch ValueError; SGD coupled L2 weight decay deferred to v0.10; STRUCTURAL CHECK NOT producer-authenticity)",
